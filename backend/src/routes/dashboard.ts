...
import { getDashboardStats, getExpiringItems, getExpiredItems, getForDeletionItems, getWillBeDeletedItems, getRecentActivity, getMissingOffers, getLongExpiryItems } from '../services/reports.js';
...
dashboard.get('/long-expiry', async (c) => {
  const items = await getLongExpiryItems();
  return c.json({ items });
});
